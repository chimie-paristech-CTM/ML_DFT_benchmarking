...
if __name__ == "__main__":
    ...
    seeds = get_seeds(args.seed, 2)

    logger.info(seeds)

    seed_max_value = [0, 0]
    for seed in seeds:
        model = RForest(n_estimators=100, max_features=0.3, min_samples_leaf=1, seed=seed)
    ...
    next_rxns = iterative_sampling(df_pool, logger, column=f"ucb {seed_max_value[0]}")
