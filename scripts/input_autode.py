...
parser.add_argument('--csv_file', type=str, default='../data/rxns_benchmarked.csv',
...
def create_ade_input(rxn_smile, idx, dir, hmet_confor=r"True"):
    ...
    final_basis_set = '6-311++G**'
    initial_basis_set = '6-31G*'
    cores = 24
    mem = 4000
    num_conf = 1000
    rmsd = 0.1

    file_name = f"ade_{idx:05}.py"
    ...
    with open(f"{dir}/{file_name}", 'w') as in_ade:
        ...
        in_ade.write(f"ade.Config.G16.keywords.opt.basis_set = '{final_basis_set}' \n")
        in_ade.write("ade.Config.G16.keywords.opt.append('tight') \n")
        in_ade.write(f"ade.Config.G16.keywords.sp.basis_set = '{final_basis_set}' \n")
        in_ade.write(f"ade.Config.G16.keywords.opt_ts.basis_set = '{final_basis_set}' \n")
        in_ade.write(f"ade.Config.G16.keywords.hess.basis_set = '{final_basis_set}' \n")
        in_ade.write(f"ade.Config.G16.keywords.low_opt.basis_set = '{initial_basis_set}' \n")
        in_ade.write(f"ade.Config.G16.keywords.low_opt.max_opt_cycles = 25\n")
